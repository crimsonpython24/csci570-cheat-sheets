...
\setlist[enumerate]{noitemsep,topsep=0pt,parsep=0pt,partopsep=0pt,
  leftmargin=!,labelindent=\blistind,labelwidth=1em,labelsep=0.3em}
...
\kp{Check uniqueness: find min-cut closest to $s$ and min-cut closest to $t$. If they are same cut, min-cut must be unique.}
...
\ap{Given bipartite graph $G$, find largest possible matching in $G$. Plan: design flow network $G'$ that has flow value $v(f){=}k$ iff $\exists$ a matching of size $k$ in $G$.}
\begin{center}
...
\kp{Fix: before extracting $s$-$t$ paths, remove all flow cycles on single edges.}
...
\ip{Proof: $\sum_v d_v{=}\sum_v(f^{\text{in}}(v){-}f^{\text{out}}(v))$. Each edge $e$ contributes ${+}f(e)$ to one node's $f^{\text{in}}$ and ${-}f(e)$ to another's $f^{\text{out}}$, so all terms cancel.}
...
\ap{Reduce feasible circulation with lower bounds to feasible circ.\ problem.}
...
\tp{Create bipartite flow network:}
\kp{$s{\to}i$: capacity $[\,m_i,\;\ell_i\,]$ {$\cdots$} bounds on questions per customer}
\kp{$i{\to}j$ ($i$ bought $j$): cap $[\,0,\;1\,]$ {$\cdots$} ask {$\le$}1 question per product per customer}
\kp{$j{\to}t$: capacity $[\,m_j,\;\ell_j\,]$ {$\cdots$} bounds on questions per product}
\tp{Lower-bounds reduction to standard max flow:}
\kp{$\forall$ edge $(u,v)$ with lower bound $\ell_{uv}$: subtract $\ell_{uv}$ from capacity, so new cap${=}c_{uv}{-}\ell_{uv}$.}
\fp{Subtracting $\ell$ breaks conservation: node $v$ ``loses'' $\sum_{\text{in}}\ell_{uv}$ incoming and ``loses'' $\sum_{\text{out}}\ell_{vw}$ outgoing. Set demand $d_v{=}\sum_{\text{in}}\ell_{uv}{-}\sum_{\text{out}}\ell_{vw}$.}
\fp{Add super-source $s^*$ and super-sink $t^*$:
$\forall v$ with $d_v{>}0$: add edge $(s^*,v)$ with cap $d_v$.
$\forall v$ with $d_v{<}0$: add edge $(v,t^*)$ with cap $|d_v|$.}
\fp{Also add edge $(t{\to}s)$ with cap $\infty$ to allow circulation in original network.}
\kp{Compute max flow in this new graph. Let $D{=}\sum_{d_v>0}d_v$.}
\fp{$v(f){=}D$: feasible survey exists. Recover assignment: $f_{uv}^{\text{original}}{=}f_{uv}^{\text{new}}{+}\ell_{uv}$.}
\fp{$v(f){<}D$: no valid survey exists.}
\begin{center}
\begin{tikzpicture}[>=Stealth, scale=0.85, every node/.style={transform shape},
  ...
  \draw[->,blue!80] (s) -- node[font=\tiny,blue!80,above,inner sep=0.5pt,xshift=-12pt,yshift=2pt]{$[m_i,\ell_i]$} (c1);
  ...
  \draw[->,red!70] (p1) -- node[font=\tiny,red!70,above,inner sep=0.5pt,xshift=3pt,yshift=3pt]{$[m_j,\ell_j]$} (t);
  \foreach \j in {2,...,3} { \draw[->,red!70] (p\j) -- (t); }
\end{tikzpicture}
\end{center}
\tp{Reduction to max flow: A feasible survey assignment exists iff max flow value $v(f)$ in the transformed network equals $D=\sum_{d_v > 0} d_v$.}
\ip{If valid survey exists, it satisfies all $\ell_e$ and $c_e$. Subtracting lower bounds gives a feasible circulation in the residual network, which corresponds to a flow of value $D$ from $s^*$ to $t^*$ since every super-source edge is saturated.}
\ip{If $v(f) = D$, then all edges from $s^*$ are saturated, meaning every node's demand is met. Adding back the lower bounds recovers a flow that respects all original capacities $[\ell_{uv}, c_{uv}]$, which encodes a valid survey assignment.}
...
Survey Design & circ.+lower bds & max-flow \\
}
