...
\kp{$\OPT(i)$ over $\{1\ldots i\}$ fails: when $n{\in}O_n$, subproblem has capacity $W{-}w_n$, not $W$, so $\OPT$ are two different functions depending on case.}
...
\tp{Polynomial: $T$ polynomial to input size (number of items). Pseudopolynomial: $T$ exponential to input's bit length.}
...
\ap{$\OPT(i,j)${=}optimal cost for $A_i\cdots A_j$; $\OPT(i,i){=}0$. $R_i${=}rows of $A_i$; $C_k${=}cols of $A_k$; $C_j${=}cols of $A_j$. Time: $O(n^3)$.}
\rb{\OPT(i,j) = \min_{i \le k < j}\bigl\{\OPT(i,k) + \OPT(k{+}1, j) + R_i C_k C_j\bigr\}}

%──────────────────────────────────────────────────────────────
\mysec{Bellman-Ford (Shortest Paths) \{Using {$G{=}(v,e)$} notation\}}
%──────────────────────────────────────────────────────────────
\tp{Directed graph, possibly negative weights. Negative cycle with path to $t$ implies dist${=}{-\infty}$. Find shortest s-t path.}
\kp{If no negative cycles: shortest path is simple (loop-free), at most $v{-}1$ edges.}
\ap{$\OPT(i,v)${=}min cost of $u{\to}v$ path using ${\le}i$ edges. Goal: find $\OPT(n{-}1,s)$.}
...
\end{code}
\ap{Variant: visit nodes closer to $t$ in number of edges to reduce passes.}
% \ip{Worst-case: $O(n^3)$. Tight bound: $\Theta(mn)$.}
\mytab{@{}l@{\extracolsep{\fill}}l@{\hspace{1.5em}}l@{}}{%
 & Bellman-Ford & Dijkstra \\
Time & $O(mn)$; $O(n^3)$ in dense graph & $O(m\log n)$ \\
Neg.\ weight{$\quad$} & yes & no \\
}
\mysubsec{Negative cycle detection}
\kp{If $\OPT(n,\{e_v\}){<}\OPT(n{-}1,\{e_v\})$ for any $\{e_v\}$: negative cycle exists.}
\kp{Problem: cycle to $t$ keeps $\infty$ distances (iteration 0), never detected.}
\kp{Fix: add super-sink $t^*$ with 0-cost edges from every node {$v$} to $t^*$. Run B-F to $t^*$: for every iteration {$i$\ [0, $v$]}, use {$i$} hops to find min path to {$t^*$}.}
\ap{With {$v$} nodes, B-F requires {$v$} iterations. If a {$u\text{-}v$} path still decrease from {$v{-}1$}-th to {$v$}-th itn., {$v$} is on negative cycle OR {$v$} has path to neg.\ cycle. In this case, DP goes toward {$t^*$}}
...
\fp{Capacity constraint: for each $e{\in}E$, $0{\le}f(e){\le}c_e$.}
\fp{Conservation of flow: {$\forall$} $v{\in}V\setminus\{s,t\}$, $\sum_{e\;\text{into}\;v}f(e){=}\sum_{e\;\text{out of}\;v}f(e)$.}
\fp{\defterm{Integrality}: if $\forall c(e){\in}\mathbb{Z}$, there {$\exists$} max flow that is integer-valued.}
...
\tp{Lemma: if ${\forall} e{\in}E\text{,}\ c(e){\in}\mathbb{Z}$, then F-F ends in ${\le}C$ itn where $C{=}\sum_{e{\leftarrow}s^*} c(e)$. {$\because$} augmentation increases flow by at least 1, {$\therefore$} max-flow $v(f) \leq C$ guarantees termination iff all edges have non-negative integer capacities.}
...
\begin{code}
Aug(f,C,P):
  b=bottleneck(P) // P:=augmenting path found in G_res; f:=flow assignment
  for each edge (v,u) in P:
    if (v,u) is forward edge: increase f(e) by b
    else (v,u) is backward edge: let e=(u,v); decrease f(e) by b
  return f'
\end{code}
\mysubsec{Ford-Fulkerson Algorithm (can solve cycles)}
\begin{code}
MaxFlow(G,s,t,C):
  init f(e)=0 for all e in G // C:=capacity func. used in bottleneck
...
Scaling_MaxFlow(G,s,t,C):
  delta=largest power of 2<=max cap in an edge
  while delta>=1:
    while there is s-t path in G_f(delta)>=delta:
      P=s-t path in G_f(delta); f'=Aug(f,C,P); f=f'; update G_f(delta)
    delta=next smallest power of 2 (e.g., 16->8)
  return f
\end{code}
\ip{Circulation feasible iff max flow{=}total demand{=$D$}.}
...
\ap{m: edges, n: vertices; flow network: BFS{$\approx$}$O(m)$}
...
\fp{$d_v{>}0$: $v$ is a sink ($d_v$ flow leaves); $d_v{<}0$: $v$ is a source ($|d_v|$ flow leaves).}
\fp{$d_v{=}0$: neither sink nor source.}
\fp{Note: a node that supplies flow has negative demand (source of flow).}
\mysubsec{Circulation $f(e)$ properties}
\kp{Capacity: for each $e{\in}E$, $0{\le}f(e){\le}c_e$.}
\kp{Demand: $\forall$ node $v$, $\sum_{e\ \text{into}\ v}f(e){-}\sum_{e\ \text{out of}\ v}f(e){=}f^{\text{in}}(v){-}f^{\text{out}}(v){=}d_v$.}
...
\ap{Reduction to max-flow: make $G'$ keep all edges/capacities from $G$. Add super-source $s^*$ and super-sink $t^*$:}
...
\kp{Push initial flow $f_0$ where $f_0(e){=}\ell_e$ through $G$ to create flow imbalance $L_v$ (find $f_0$ to satisfy all $\ell_e$'s), where total demand {$D$}{=}{$\sum_{\text{pos} L_v}$}{=}{$\lvert\sum_{\text{neg} L_v}\rvert$}:}
\ip{$L_v = \sum_{e\;\text{into}\;v}\!\ell_e \;-\; \sum_{e\;\text{out of}\;v}\!\ell_e$; disregard {$d_v$} in this step.}
\kp{Construct $G'$ with $c'_e{=}c_e{-}\ell_e$, $\ell'_e{=}0$, and $d'_v{=}d_v{-}L_v$.}
\kp{If asked, find feasible circ.\ $f_1$ in $G'$ using max-flow reduction \& remaining capacity. If none exists, no feasible circulation in $G$. Otherwise, $G{=}f_0{+}f_1$.}
